...
| P6 Activation | DONE (test mode) — September 19, 2026 | Rules deployed to `not-the-rug`; TTL `expiresAt` ACTIVE on `analytics_events` and `analyticsRateLimits`; Vercel preview + production set to `NEXT_PUBLIC_ANALYTICS_ENABLED=true`, `NEXT_PUBLIC_ANALYTICS_TEST_MODE=true`; production deployed from `main` (see plans/010 release evidence). | Real collection is NOT started: production stays in test mode until the owner separately approves redeploying with `NEXT_PUBLIC_ANALYTICS_TEST_MODE=false`. |
