      ...
      {renders.map((r, i) => (
        <figure key={r.id} className={`polaroid ${TILT_CLASSES[i % TILT_CLASSES.length]}`} style={{ margin: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 8, paddingTop: 10 }}>
            ...
            <button type="button" className="btn btn-primary booking-forward-btn btn-sm admin-btn-secondary" onClick={() => onReRender(r)}>Re-render</button>
            <button type="button" className="btn btn-primary booking-forward-btn btn-sm admin-btn-secondary" onClick={() => onDelete(r)}>
