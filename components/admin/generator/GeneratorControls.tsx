    ...
    <div id="admin-gen-controls-grid">
      ...
      <div className="card card-pad" id="admin-gen-section-logo">
        ...
        <div id="admin-gen-size-row" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <button type="button" className="btn btn-primary booking-forward-btn btn-sm admin-btn-secondary" onClick={onResetPlacement}>Reset</button>
        </div>
        ...
        <div id="admin-gen-source-seg" style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          {(['random', 'selected'] as SourceMode[]).map((m) => (
            <button
              key={m}
              type="button"
              className={`btn btn-primary booking-forward-btn btn-sm ${sourceMode === m ? 'btn-accent' : 'admin-btn-secondary'}`}
        ...
        {sourceMode === 'random' && (
          uploads.length === 0 ? (
            ...
            <div id="admin-gen-random-row" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button type="button" className="btn btn-primary booking-forward-btn btn-sm admin-btn-secondary" onClick={onShuffle}>Shuffle</button>
            </div>
          ) : (
